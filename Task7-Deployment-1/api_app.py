...
REQ_CNT = Counter("req_count", "HTTP requests", ["method", "path", "status"])
REQ_LAT = Histogram("req_latency_sec", "Latency", ["method", "path"])
...
async def log_requests(request: Request, call_next):
    ...
    try:
        ...
        logger.error(f"! 500 {m} {p} :: {exc}")
        REQ_CNT.labels(m, p, "500").inc()        # NEW
        raise

    latency = time.time() - start                # NEW
    REQ_LAT.labels(m, p).observe(latency)        # NEW
    REQ_CNT.labels(m, p, str(resp.status_code)).inc()  # NEW
    logger.info(f"<< {resp.status_code} {m} {p} [{latency:.3f}s]")
    return resp
